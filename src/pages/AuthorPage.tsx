...
const AuthorPage: React.FC = () => {
	...
	useEffect(() => {
		const loadCourses = async () => {
			if (walletState.isConnected && walletState.address) {
				...
				try {
					fetch(
						`${FETCH_URL}/api/getUserCourses?address=${walletState.address}`,
					)
						.then((res) => res.json())
						.then((data) => {
							if (data.code === 200) {
	...
	return (
		<div className="space-y-6 mt-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{isLoadingCourses ? (
						...
						createdCourses.map((course) => (
							<Card
								key={course.courseId}
								className="bg-purple-50 border-purple-200 h-96 flex flex-col"
							>
						...
						<div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
							...
							<div className="mb-6">
								...
								<div className="flex items-center space-x-4 text-sm text-muted-foreground mb-4">
									...
									<span>
										创建时间:{" "}
										{new Date(previewCourse.createdAt).toLocaleDateString()}
									</span>
								</div>
			...
			>
				<DialogContent className="sm:max-w-md">
							...
							{verificationMessage && (
								<div
									className={`text-sm p-2 rounded ${
										verificationMessage.includes("成功")
					...
					<div className="flex justify-end space-x-3">
						{!isVerifying ? (
							<>
								<Button variant="outline" onClick={handleCancelEditUsername}>
									取消
								</Button>
								<Button onClick={handleSaveUsername}>保存并验证</Button>
							</>
						) : (
			...
			{/* 自动验证对话框 */}
			<Dialog
				open={showAutoVerifyDialog}
				onOpenChange={setShowAutoVerifyDialog}
			>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						<DialogTitle className="flex items-center space-x-2">
						...
						<div className="space-y-2">
							<div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
								<p className="text-sm font-medium text-blue-800">
									当前用户名:{" "}
									{walletState.username === walletState.address
										? formatAddressDisplay(walletState.username || "") +
											" (地址)"
